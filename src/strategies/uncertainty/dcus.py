...
class DCUSStrategy(BaseStrategy):
    ...
    def _ensure_classwise_quality(self):
        ...
        if self.experiment_dir:
            ...
            train_dir = Path(self.experiment_dir) / f"round_{self.round}" / 'train'
            last_ckpt_path = next(train_dir.glob("*/weights/last.pt"), None)
            classwise_quality_npy_path = next(train_dir.glob("*/weights/classwise_quality.npy"), None)
            ...
            elif classwise_quality_npy_path is not None and classwise_quality_npy_path.exists():
